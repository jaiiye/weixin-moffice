...
public class Share extends Model<Share> {
	...
	public Page<Share> paginate(int pageNumber, int pageSize) {
		return paginate(pageNumber, pageSize, "select *", "from wx_share order by id asc");
	}
	
	public Page<Share> paginateImage(int pageNumber, int pageSize) {
		return paginate(pageNumber, pageSize, "select *", "from wx_share where msgType='image' order by id asc");
	}
}
